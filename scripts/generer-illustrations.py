...
# Le site est clair de bout en bout : les illustrations le sont aussi. Ce qui
# était clair sur fond nuit est devenu sombre sur fond blanc, et réciproquement.
# Les noms disent le rôle, pas la teinte : ils restent valables si la charte
# change à nouveau.
FOND_1, FOND_2 = "#FFFFFF", "#E4F3F0"      # dégradé de fond, très clair
ENCRE = "#14231F"                          # détails les plus sombres
VERT, VERT_CLAIR, VERT_PALE = "#0F766E", "#14B8A6", "#0D9488"
ORANGE, ORANGE_CLAIR = "#EA580C", "#FB923C"
ACIER_1, ACIER_2 = "#94A3B8", "#6B7F84"    # métal, lisible sur fond clair
TRAIT = "#3F534D"                          # traits fins : outils, crochets

L, H = 1200, 750          # format des visuels de carte (16/10)


def entete(titre: str, largeur=L, hauteur=H) -> str:
    """Fond commun : dégradé clair, halo vert d'eau, grille technique discrète."""
  ...
  <defs>
    <linearGradient id="fond" x1="0" y1="0" x2="1" y2="1">
      <stop offset="0" stop-color="{FOND_1}"/><stop offset="1" stop-color="{FOND_2}"/>
    </linearGradient>
    <radialGradient id="halo" cx="0.72" cy="0.18" r="0.75">
      <stop offset="0" stop-color="{VERT_CLAIR}" stop-opacity="0.20"/>
      <stop offset="1" stop-color="{VERT_CLAIR}" stop-opacity="0"/>
    </radialGradient>
    <linearGradient id="acier" x1="0" y1="0" x2="1" y2="1">
      <stop offset="0" stop-color="#B7C4C6"/><stop offset="1" stop-color="{ACIER_2}"/>
    </linearGradient>
    <linearGradient id="cuivre" x1="0" y1="0" x2="1" y2="1">
      <stop offset="0" stop-color="{ORANGE_CLAIR}"/><stop offset="1" stop-color="#C2410C"/>
    </linearGradient>
    <linearGradient id="porte" x1="0" y1="0" x2="1" y2="1">
      <stop offset="0" stop-color="#DCE8E7"/><stop offset="1" stop-color="#BCCFCC"/>
    </linearGradient>
    <pattern id="grille" width="60" height="60" patternUnits="userSpaceOnUse">
      <path d="M60 0H0V60" fill="none" stroke="#0F766E" stroke-opacity="0.06" stroke-width="1"/>
    </pattern>
  </defs>
...
def porte(x, y, w, h, entrouverte=False, blindee=False, panneaux=True):
    ...
    if blindee:
        s += f'<rect x="{x+10}" y="{y+10}" width="{w-20}" height="{h-20}" rx="3" fill="none" stroke="{VERT}" stroke-opacity="0.5" stroke-width="3"/>'
        for i in range(1, 5):
            s += f'<circle cx="{x+26}" cy="{y+40*i+20}" r="4" fill="{VERT_PALE}" opacity="0.55"/>'
    if panneaux:
        s += (f'<rect x="{x+34}" y="{y+38}" width="{w-68}" height="{h*0.34:.0f}" rx="4" fill="none" stroke="{VERT}" stroke-opacity="0.34" stroke-width="3"/>'
              f'<rect x="{x+34}" y="{y+h*0.46:.0f}" width="{w-68}" height="{h*0.44:.0f}" rx="4" fill="none" stroke="{VERT}" stroke-opacity="0.34" stroke-width="3"/>')
    s += '</g>'
    return s
...
def cylindre(cx, cy, r=30, actif=True):
    ...
    return (f'<circle cx="{cx}" cy="{cy}" r="{r+9}" fill="{ACIER_2}"/>'
            ...
            f'<circle cx="{cx}" cy="{cy}" r="{r*0.34:.0f}" fill="{ENCRE}"/>'
            f'<rect x="{cx-3}" y="{cy}" width="6" height="{r*0.8:.0f}" rx="3" fill="{ENCRE}"/>')
...
def sol(y, x1=60, x2=L-60):
    return f'<rect x="{x1}" y="{y}" width="{x2-x1}" height="14" rx="4" fill="#AEC5C1"/>'
...
def outil_tournevis(x, y, angle=-30):
    return (f'<g transform="translate({x} {y}) rotate({angle})">'
            ...
            f'<rect x="86" y="-5" width="80" height="10" rx="3" fill="{ACIER_1}"/>'
            f'<rect x="160" y="-7" width="16" height="14" rx="2" fill="{TRAIT}"/>'
            '</g>')


def crochet(x, y, angle=0):
    return (f'<g transform="translate({x} {y}) rotate({angle})" fill="none" stroke="{TRAIT}" '
...
def caisse_outils(x, y):
    return (f'<g transform="translate({x} {y})">'
            ...
            f'<rect x="20" y="30" width="52" height="11" rx="5" fill="#B7C4C6"/>'
            f'<rect x="20" y="56" width="88" height="11" rx="5" fill="#CBD8D9"/>'
            '</g>')
...
def badge_texte(x, y, texte, couleur=VERT_PALE):
    largeur = 26 + len(texte) * 15
    return (f'<g transform="translate({x} {y})">'
            f'<rect x="0" y="0" width="{largeur}" height="46" rx="23" fill="#FFFFFF" fill-opacity="0.92" '
            f'stroke="{couleur}" stroke-opacity="0.55" stroke-width="2"/>'
...
def il_hero():
    ...
    s += etincelles(700, 470, VERT_PALE)
    s += badge_texte(96, 96, "SERRURERIE", VERT_PALE)
    s += sol(730, 60, 1040)
    return s + PIED
...
def il_ouverture_porte():
    ...
    s += (f'<g transform="translate(694 402) rotate(-7)">'
          f'<rect width="176" height="112" rx="9" fill="#FFFFFF" opacity="0.95"/>'
          f'<rect width="176" height="24" rx="9" fill="{ORANGE}"/>'
          f'<rect x="16" y="46" width="96" height="9" rx="4" fill="#C3D3D0"/>'
          f'<rect x="16" y="68" width="136" height="9" rx="4" fill="#DEEAE7"/></g>')
    s += f'<path d="M694 430 L664 420" stroke="#FFFFFF" stroke-width="8" stroke-linecap="round"/>'
...
def il_porte_bloquee():
    ...
    s += cle(696, 452, 20, couleur="#6B7F84", echelle=0.55)
...
def il_cle_cassee():
    ...
    s += f'<circle cx="600" cy="380" r="240" fill="{VERT}" opacity="0.07"/>'
    s += cylindre(600, 380, 128)
    # Fragment resté engagé
    s += f'<rect x="594" y="380" width="13" height="104" rx="5" fill="#6B7F84"/>'
    s += f'<path d="M600 484 l-9 22 l18 0 z" fill="#8C9EA2"/>'
    # Extracteur
    ...
    s += cle(160, 600, -18, couleur="#6B7F84", echelle=0.85)
...
def il_cle_perdue():
    ...
    s += cle(238, 430, 12, couleur="#6B7F84", echelle=0.8)
...
def il_changement_serrure():
    ...
    s += (f'<g transform="translate(840 300)">'
          ...
          f'<circle cx="52" cy="36" r="9" fill="{ENCRE}"/>'
          f'<rect x="94" y="28" width="92" height="16" rx="6" fill="#B7C4C6"/></g>')
...
def il_installation_serrure():
    ...
    # Tracé de pose, cotes
    s += (f'<g stroke="{VERT_PALE}" stroke-width="3" stroke-dasharray="10 8" opacity="0.75" fill="none">'
          f'<path d="M300 400 H720"/><path d="M620 70 V670"/></g>')
    s += (f'<g stroke="{VERT_PALE}" stroke-width="3" opacity="0.9">'
          f'<path d="M740 400 v-120 M732 292 l8 -14 8 14 M732 388 l8 14 8 -14"/></g>')
    s += f'<text x="768" y="352" fill="{VERT_PALE}" font-family="system-ui,sans-serif" font-size="26" font-weight="700">axe</text>'
...
def il_serrure_3_points():
    ...
    for cy in (170, 370, 570):
        ...
        s += f'<circle cx="700" cy="{cy}" r="11" fill="{VERT_PALE}" opacity="0.65"/>'
    # Tringlerie verticale
    s += f'<rect x="694" y="170" width="12" height="400" rx="6" fill="{VERT}" opacity="0.5"/>'
...
def il_serrure_5_points():
    ...
    for cy in (120, 260, 380, 500, 636):
        ...
        s += f'<circle cx="700" cy="{cy}" r="10" fill="{VERT_PALE}" opacity="0.65"/>'
    s += f'<rect x="694" y="120" width="12" height="516" rx="6" fill="{VERT}" opacity="0.5"/>'
...
def il_porte_blindee():
    ...
    s += f'<rect x="756" y="150" width="10" height="450" rx="5" fill="{VERT}" opacity="0.45"/>'
...
def il_blindage_porte():
    ...
    s += (f'<g fill="{VERT_PALE}" opacity="0.45">' +
...
def il_effraction():
    ...
    # Zone arrachée
    s += f'<rect x="656" y="330" width="52" height="140" rx="8" fill="{ENCRE}"/>'
    ...
    s += (f'<g transform="translate(830 250)">'
          f'<rect width="180" height="240" rx="12" fill="#FFFFFF" opacity="0.95"/>'
    ...
    s += badge_texte(80, 596, "MISE EN SÉCURITÉ", "#047857")
    s += sol(678, 60, 1040)
    return s + PIED
...
def il_coffre_fort():
    ...
    s += f'<rect x="326" y="146" width="428" height="408" rx="10" fill="#3B5250"/>'
    s += f'<rect x="340" y="160" width="360" height="380" rx="8" fill="url(#porte)"/>'
    # Cadran
    s += f'<circle cx="470" cy="350" r="76" fill="{ENCRE}"/>'
    s += f'<circle cx="470" cy="350" r="58" fill="none" stroke="url(#cuivre)" stroke-width="9"/>'
    s += (f'<g stroke="{VERT_PALE}" stroke-width="5" stroke-linecap="round">'
          f'<path d="M470 286v16M470 398v16M406 350h16M518 350h16"/></g>')
    s += f'<circle cx="470" cy="350" r="15" fill="url(#cuivre)"/>'
    s += f'<path d="M470 350 l42 -32" stroke="#FFFFFF" stroke-width="9" stroke-linecap="round"/>'
    # Poignée et pênes
    ...
    s += (f'<g transform="translate(850 300)" fill="none" stroke="#FFFFFF" stroke-width="9" stroke-linecap="round">'
          f'<path d="M0 0 q46 66 4 122"/><circle cx="4" cy="140" r="23" fill="{ENCRE}" stroke="url(#cuivre)"/></g>')
...
def il_rideau_metallique():
    ...
    s += f'<rect x="168" y="104" width="864" height="12" fill="#8C9EA2"/>'
    ...
    s += ('<g stroke="#A9BABD" stroke-width="3">' +
          "".join(f'<path d="M198 {y}h804"/>' for y in range(146, 416, 30)) + '</g>')
    s += f'<rect x="198" y="398" width="804" height="22" rx="5" fill="#9AACAF"/>'
    # Serrure centrale forcée
    s += f'<circle cx="600" cy="410" r="24" fill="{ENCRE}"/>'
...
def il_devis():
    ...
    s += (f'<g transform="translate(340 110) rotate(-3)">'
          f'<rect width="440" height="560" rx="16" fill="#FFFFFF"/>'
          f'<rect width="440" height="70" rx="16" fill="{VERT}"/>'
          f'<rect x="40" y="120" width="240" height="16" rx="8" fill="#C3D3D0"/>'
          f'<rect x="40" y="158" width="360" height="12" rx="6" fill="#DEEAE7"/>')
    for i in range(5):
        y = 220 + i * 54
        s += (f'<rect x="40" y="{y}" width="200" height="13" rx="6" fill="#DEEAE7"/>'
              f'<rect x="308" y="{y}" width="92" height="13" rx="6" fill="{VERT_PALE}" opacity="0.6"/>')
